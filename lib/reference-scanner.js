...
function pushScannableChunk(chunks, value) {
  if (!value) return;
...
function filterMdContent(raw) {
  ...
  let tokens;
  try {
    tokens = markdownParser.parse(raw, {});
  } catch (_e) {
    return filterMdContentLegacy(raw);
  }
  ...
  for (const token of tokens) {
    ...
    if (headingDepth > 0 || tableDepth > 0) {
      continue;
    }

    if (token.type === "inline") {
      pushScannableChunk(chunks, token.content);
      continue;
    }
...
function filterMdContentLegacy(raw) {
  let fenceLen = 0;
