...
async function install(source, pluginFilter, projectRoot = process.cwd(), options = {}) {
  ...
  try {
    ...
    if (existing) {
      ...
      uninstallFlatPluginFiles(path.join(projectRoot, OPENCODE_PLUGINS_DIR), existing.hooks);
    ...
    for (const plugin of plugins) {
      ...
      const targetPluginsRoot = path.join(projectRoot, OPENCODE_PLUGINS_DIR);
      for (const hookSourceDir of ["plugin", "plugins"]) {
        const sourceHooksDir = path.join(pluginRoot, hookSourceDir);
        const installedHooks = installFlatPluginDir(
          sourceHooksDir, targetPluginsRoot, registry,
          {
            marketplaceName, force,
            currentOwnedNames: allHooks,
            sourceRoot: marketplaceRoot,
            strictSymlinkBoundary: true,
          },
        );
        for (const hookName of installedHooks) {
          pushUnique(allHooks, hookName);
        }
      }
    }
...
function uninstall(name, projectRoot = process.cwd()) {
  ...
  uninstallFlatPluginFiles(path.join(projectRoot, OPENCODE_PLUGINS_DIR), entry.hooks);
