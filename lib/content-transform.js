...
function normalizeRewritePrefix(prefix) {
  if (prefix.endsWith("./")) return prefix.slice(0, -2);
...
function rewriteFilePaths(content, rewriteCtx) {
  ...
  if (pluginSourceNorm) {
    ...
    content = content.replace(fullPattern, (_match, prefix) => {
      return `${normalizeRewritePrefix(prefix)}${targetPrefix}`;
    });
  }
  ...
  for (const dir of copiedDirs) {
    ...
    content = content.replace(relPattern, (_match, prefix, rel) => {
      return `${normalizeRewritePrefix(prefix)}${targetPrefix}${rel}`;
    });
  }
